...
in {
  ...
  config = mkIf cfg.enable {
    systemd.services.OBORmarathon = {
      ...
      serviceConfig = {
        TimeoutStartSec = "0";
        ExecStartPre = "${pkgs.docker}/bin/docker pull mesosphere/marathon:v1.5.8";
        ExecStart = "${pkgs.docker}/bin/docker run --rm --name=marathon --net=host mesosphere/marathon:v1.5.8 --master ${cfg.master} --zk zk://${concatStringsSep "," cfg.zookeeperHosts}/marathon --http_port ${toString cfg.httpPort} ${concatStringsSep " " cfg.extraCmdLineOptions}";
