...
sudo /run/current-system/sw/bin/nix-store --add-fixed sha256 jdk-8u111-linux-x64.tar.gz
...
sudo /run/current-system/sw/bin/nix-store --add-fixed sha256 jdk-8u121-linux-x64.tar.gz
...
sudo /run/current-system/sw/bin/nix-store --add-fixed sha256 jdk-8u131-linux-x64.tar.gz
...
set -e
sudo /run/current-system/sw/bin/nixos-rebuild build  -I nixpkgs=/nixpkgs/
sudo /run/current-system/sw/bin/nixos-rebuild switch  -I nixpkgs=/nixpkgs/
